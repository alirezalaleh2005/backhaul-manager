...
#  Version : 1.1.0
#  Author  : https://github.com/alirezalaleh2005/
#  Supports: TCP | TCPMUX | WSMUX | WSSMUX
#  Roles   : Iran (Server) | Kharej (Client)
#  Features: Direct SSH Config Sync, Advanced Monitoring, Preset Tuning
# =============================================================================
set -uo pipefail
...
SYNC_SSH_KEY="$INSTALL_DIR/.sync_key"
...
_setup_ssh_key() {
    if [[ ! -f "$SYNC_SSH_KEY" ]]; then
        info "Generating dedicated SSH key for sync..."
        ssh-keygen -t ed25519 -f "$SYNC_SSH_KEY" -N "" -q || die "Failed to generate SSH key."
        success "Sync key generated at $SYNC_SSH_KEY"
    fi
}

_sync_config_to_peer() {
    local local_cfg="$1"
    local peer_ip="$2"
    local svc_name="$3"
    ...
    info "Testing SSH connection to $peer_ip..."
    if ! ssh -i "$SYNC_SSH_KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
             -p "$SYNC_REMOTE_PORT" "${SYNC_REMOTE_USER}@${peer_ip}" "echo ok" &>/dev/null; then
        
        warn "SSH access denied or unreachable."
        echo -e "  ${DIM}Please copy this public key to the peer server manually:${NC}"
        echo -e "  ${LYELLOW}$(cat "${SYNC_SSH_KEY}.pub")${NC}"
        echo -e "  ${DIM}Run on peer: mkdir -p ~/.ssh && echo 'KEY_ABOVE' >> ~/.ssh/authorized_keys${NC}"
        press_enter
        return 1
    fi

    info "Syncing config to $peer_ip:$INSTALL_DIR/..."
    scp -i "$SYNC_SSH_KEY" -o StrictHostKeyChecking=no -P "$SYNC_REMOTE_PORT" \
        "$local_cfg" "${SYNC_REMOTE_USER}@${peer_ip}:${INSTALL_DIR}/$(basename "$local_cfg")" \
        || { warn "Failed to transfer config file."; return 1; }

    info "Restarting service '$svc_name' on peer..."
    ssh -i "$SYNC_SSH_KEY" -o StrictHostKeyChecking=no -p "$SYNC_REMOTE_PORT" \
        "${SYNC_REMOTE_USER}@${peer_ip}" "systemctl restart '$svc_name' && systemctl is-active --quiet '$svc_name'" \
        || { warn "Peer service failed to start after sync."; return 1; }

    success "Config synced and service restarted on $peer_ip successfully."
    return 0
}

menu_sync_configs() {
    section "Direct Config Sync (SSH)"
    check_binary || return
    ...
    if [[ ${#configs[@]} -eq 0 ]]; then
        warn "No config files found to sync."
        press_enter; return
    fi

    echo -e "  ${BOLD}${WHITE}Select config to sync:${NC}"
    local i=1
    for cfg in "${configs[@]}"; do
        echo -e "  ${WHITE}[$i]${NC} ${CYAN}$(basename "$cfg")${NC}"
        ((i++))
    done
    echo -e "  ${WHITE}[0]${NC} Back"
    separator
    prompt "Choice:"; read -r sel
    [[ "$sel" == "0" ]] && return
    ...
    local chosen="${configs[$((sel-1))]}"
    prompt "Enter PEER server IP address:"; read -r peer_ip
    [[ -z "$peer_ip" ]] && { warn "IP cannot be empty."; return; }
    
    local base; base=$(basename "$chosen" .toml)
    local target_svc="backhaul-${base}.service"
    
    echo ""
    echo -e "  ${BULLET} Source : ${DIM}$chosen${NC}"
    echo -e "  ${BULLET} Target : ${WHITE}${peer_ip}:${INSTALL_DIR}/${base}.toml${NC}"
    echo -e "  ${BULLET} Service: ${CYAN}$target_svc${NC}"
    separator
    prompt "Confirm sync? [y/N]:"; read -r confirm
    [[ "${confirm,,}" != "y" ]] && return
    
    _sync_config_to_peer "$chosen" "$peer_ip" "$target_svc"
    press_enter
}
...
ask_server_role() {
    clear
    _print_logo
    echo -e "  ${DIM}Backhaul Free Tunnel Manager v1.1.0 by ${NC}${CYAN}علیرضا لاله${NC}"
    echo -e "  ${DIM}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
_ask_advanced_iran() {
    local transport="$1"
    prompt "keepalive_period [${PRESET_IRAN_KEEPALIVE}]:"; read -r v; ADV_KEEPALIVE="${v:-$PRESET_IRAN_KEEPALIVE}"
    prompt "nodelay [${PRESET_IRAN_NODELAY}]:"; read -r v; ADV_NODELAY="${v:-$PRESET_IRAN_NODELAY}"
    prompt "heartbeat [${PRESET_IRAN_HEARTBEAT}]:"; read -r v; ADV_HEARTBEAT="${v:-$PRESET_IRAN_HEARTBEAT}"
    prompt "channel_size [${PRESET_IRAN_CHANNEL_SIZE}]:"; read -r v; ADV_CHANNEL_SIZE="${v:-$PRESET_IRAN_CHANNEL_SIZE}"
    
    if [[ "$transport" != "tcp" ]]; then
        prompt "mux_con [${PRESET_IRAN_MUX_CON}]:"; read -r v; ADV_MUX_CON="${v:-$PRESET_IRAN_MUX_CON}"
        prompt "mux_version [${PRESET_IRAN_MUX_VERSION}]:"; read -r v; ADV_MUX_VERSION="${v:-$PRESET_IRAN_MUX_VERSION}"
        prompt "mux_framesize [${PRESET_IRAN_MUX_FRAMESIZE}]:"; read -r v; ADV_MUX_FRAMESIZE="${v:-$PRESET_IRAN_MUX_FRAMESIZE}"
        prompt "mux_recievebuffer [${PRESET_IRAN_MUX_RECVBUF}]:"; read -r v; ADV_MUX_RECVBUF="${v:-$PRESET_IRAN_MUX_RECVBUF}"
        prompt "mux_streambuffer [${PRESET_IRAN_MUX_STREAMBUF}]:"; read -r v; ADV_MUX_STREAMBUF="${v:-$PRESET_IRAN_MUX_STREAMBUF}"
    else
        ADV_MUX_CON="$PRESET_IRAN_MUX_CON"; ADV_MUX_VERSION="$PRESET_IRAN_MUX_VERSION"
        ADV_MUX_FRAMESIZE="$PRESET_IRAN_MUX_FRAMESIZE"; ADV_MUX_RECVBUF="$PRESET_IRAN_MUX_RECVBUF"
    ...
    prompt "log_level [${_def_ll_iran}]:"; read -r v; ADV_LOG_LEVEL="${v:-$_def_ll_iran}"
    prompt "mss [${PRESET_IRAN_MSS}]:"; read -r v; ADV_MSS="${v:-$PRESET_IRAN_MSS}"
    prompt "so_rcvbuf [${PRESET_IRAN_SO_RCVBUF}]:"; read -r v; ADV_SO_RCVBUF="${v:-$PRESET_IRAN_SO_RCVBUF}"
    prompt "so_sndbuf [${PRESET_IRAN_SO_SNDBUF}]:"; read -r v; ADV_SO_SNDBUF="${v:-$PRESET_IRAN_SO_SNDBUF}"
    prompt "sniffer (true/false) [${PRESET_IRAN_SNIFFER}]:"; read -r v; ADV_SNIFFER="${v:-$PRESET_IRAN_SNIFFER}"
    prompt "web_port (0=disable) [${PRESET_IRAN_WEB_PORT}]:"; read -r v; ADV_WEB_PORT="${v:-$PRESET_IRAN_WEB_PORT}"
}

_ask_advanced_kharej() {
    local transport="$1"
    prompt "connection_pool [${PRESET_KHAREJ_CONN_POOL}]:"; read -r v; ADV_CONN_POOL="${v:-$PRESET_KHAREJ_CONN_POOL}"
    prompt "aggressive_pool (true/false) [${PRESET_KHAREJ_AGGRESSIVE_POOL}]:"; read -r v; ADV_AGGRESSIVE_POOL="${v:-$PRESET_KHAREJ_AGGRESSIVE_POOL}"
    prompt "keepalive_period [${PRESET_KHAREJ_KEEPALIVE}]:"; read -r v; ADV_KEEPALIVE="${v:-$PRESET_KHAREJ_KEEPALIVE}"
    prompt "dial_timeout [${PRESET_KHAREJ_DIAL_TIMEOUT}]:"; read -r v; ADV_DIAL_TIMEOUT="${v:-$PRESET_KHAREJ_DIAL_TIMEOUT}"
    prompt "retry_interval [${PRESET_KHAREJ_RETRY_INTERVAL}]:"; read -r v; ADV_RETRY_INTERVAL="${v:-$PRESET_KHAREJ_RETRY_INTERVAL}"
    prompt "nodelay (true/false) [${PRESET_KHAREJ_NODELAY}]:"; read -r v; ADV_NODELAY="${v:-$PRESET_KHAREJ_NODELAY}"
    
    if [[ "$transport" != "tcp" ]]; then
        prompt "mux_version [${PRESET_KHAREJ_MUX_VERSION}]:"; read -r v; ADV_MUX_VERSION="${v:-$PRESET_KHAREJ_MUX_VERSION}"
        prompt "mux_framesize [${PRESET_KHAREJ_MUX_FRAMESIZE}]:"; read -r v; ADV_MUX_FRAMESIZE="${v:-$PRESET_KHAREJ_MUX_FRAMESIZE}"
        prompt "mux_recievebuffer [${PRESET_KHAREJ_MUX_RECVBUF}]:"; read -r v; ADV_MUX_RECVBUF="${v:-$PRESET_KHAREJ_MUX_RECVBUF}"
        prompt "mux_streambuffer [${PRESET_KHAREJ_MUX_STREAMBUF}]:"; read -r v; ADV_MUX_STREAMBUF="${v:-$PRESET_KHAREJ_MUX_STREAMBUF}"
    else
        ADV_MUX_VERSION="$PRESET_KHAREJ_MUX_VERSION"; ADV_MUX_FRAMESIZE="$PRESET_KHAREJ_MUX_FRAMESIZE"
        ADV_MUX_RECVBUF="$PRESET_KHAREJ_MUX_RECVBUF"; ADV_MUX_STREAMBUF="$PRESET_KHAREJ_MUX_STREAMBUF"
    fi
    
    local _def_ll_kharej; _def_ll_kharej=$([[ "$transport" == "tcp" ]] && echo "$PRESET_KHAREJ_LOG_LEVEL_TCP" || echo "$PRESET_KHAREJ_LOG_LEVEL_MUX")
    echo -e "  ${DIM}log_level options: panic | fatal | error | warn | info | debug | trace${NC}"
    prompt "log_level [${_def_ll_kharej}]:"; read -r v; ADV_LOG_LEVEL="${v:-$_def_ll_kharej}"
    prompt "mss [${PRESET_KHAREJ_MSS}]:"; read -r v; ADV_MSS="${v:-$PRESET_KHAREJ_MSS}"
    prompt "so_rcvbuf [${PRESET_KHAREJ_SO_RCVBUF}]:"; read -r v; ADV_SO_RCVBUF="${v:-$PRESET_KHAREJ_SO_RCVBUF}"
    prompt "so_sndbuf [${PRESET_KHAREJ_SO_SNDBUF}]:"; read -r v; ADV_SO_SNDBUF="${v:-$PRESET_KHAREJ_SO_SNDBUF}"
    prompt "sniffer (true/false) [${PRESET_KHAREJ_SNIFFER}]:"; read -r v; ADV_SNIFFER="${v:-$PRESET_KHAREJ_SNIFFER}"
    prompt "web_port (0=disable) [${PRESET_KHAREJ_WEB_PORT}]:"; read -r v; ADV_WEB_PORT="${v:-$PRESET_KHAREJ_WEB_PORT}"
}
...
menu_create_tunnel() {
    ...
    if [[ "$ROLE" == "iran" ]]; then
        ...
        ADV_KEEPALIVE="$PRESET_IRAN_KEEPALIVE"; ADV_NODELAY="$PRESET_IRAN_NODELAY"
        ADV_HEARTBEAT="$PRESET_IRAN_HEARTBEAT"; ADV_CHANNEL_SIZE="$PRESET_IRAN_CHANNEL_SIZE"
        ADV_MUX_CON="$PRESET_IRAN_MUX_CON"; ADV_MUX_VERSION="$PRESET_IRAN_MUX_VERSION"
        ADV_MUX_FRAMESIZE="$PRESET_IRAN_MUX_FRAMESIZE"; ADV_MUX_RECVBUF="$PRESET_IRAN_MUX_RECVBUF"
        ADV_MUX_STREAMBUF="$PRESET_IRAN_MUX_STREAMBUF"; ADV_LOG_LEVEL="$_log_level_default"
        ADV_MSS="$PRESET_IRAN_MSS"; ADV_SO_RCVBUF="$PRESET_IRAN_SO_RCVBUF"
        ADV_SO_SNDBUF="$PRESET_IRAN_SO_SNDBUF"; ADV_SNIFFER="$PRESET_IRAN_SNIFFER"
        ADV_WEB_PORT="$PRESET_IRAN_WEB_PORT"
        ADV_CONN_POOL="$PRESET_KHAREJ_CONN_POOL"; ADV_AGGRESSIVE_POOL="$PRESET_KHAREJ_AGGRESSIVE_POOL"
        ADV_DIAL_TIMEOUT="$PRESET_KHAREJ_DIAL_TIMEOUT"; ADV_RETRY_INTERVAL="$PRESET_KHAREJ_RETRY_INTERVAL"
    else
        _log_level_default=$([[ "$TRANSPORT" == "tcp" ]] && echo "$PRESET_KHAREJ_LOG_LEVEL_TCP" || echo "$PRESET_KHAREJ_LOG_LEVEL_MUX")
        ADV_CONN_POOL="$PRESET_KHAREJ_CONN_POOL"; ADV_AGGRESSIVE_POOL="$PRESET_KHAREJ_AGGRESSIVE_POOL"
        ADV_KEEPALIVE="$PRESET_KHAREJ_KEEPALIVE"; ADV_DIAL_TIMEOUT="$PRESET_KHAREJ_DIAL_TIMEOUT"
        ADV_RETRY_INTERVAL="$PRESET_KHAREJ_RETRY_INTERVAL"; ADV_NODELAY="$PRESET_KHAREJ_NODELAY"
        ADV_MUX_VERSION="$PRESET_KHAREJ_MUX_VERSION"; ADV_MUX_FRAMESIZE="$PRESET_KHAREJ_MUX_FRAMESIZE"
        ADV_MUX_RECVBUF="$PRESET_KHAREJ_MUX_RECVBUF"; ADV_MUX_STREAMBUF="$PRESET_KHAREJ_MUX_STREAMBUF"
        ADV_LOG_LEVEL="$_log_level_default"; ADV_MSS="$PRESET_KHAREJ_MSS"
        ADV_SO_RCVBUF="$PRESET_KHAREJ_SO_RCVBUF"; ADV_SO_SNDBUF="$PRESET_KHAREJ_SO_SNDBUF"
        ADV_SNIFFER="$PRESET_KHAREJ_SNIFFER"; ADV_WEB_PORT="$PRESET_KHAREJ_WEB_PORT"
        ADV_HEARTBEAT="$PRESET_IRAN_HEARTBEAT"; ADV_CHANNEL_SIZE="$PRESET_IRAN_CHANNEL_SIZE"
...
main_menu() {
    while true; do
        ...
        echo -e "  ${LBLUE}[5]${NC}  Sync Config to Peer ${DIM}(Direct SSH)${NC}"
